...
- **Sparklines:** the dashboard samples height + fee once a minute into an
  ...
  Served at `/api/history` (behind the same optional auth) if you want the
  raw JSON. The tower's "blocks today" count is computed separately, from the
  node's own block timestamps, so it's correct immediately (and survives a
  restart).
